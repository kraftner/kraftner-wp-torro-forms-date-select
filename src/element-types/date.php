...
class Date extends Element_Type {
	...
	public function filter_json( $data, $element, $submission = null ) {
		...
		$data['date_labels'] = array(
			'year'  => __( 'Year', 'kraftner-wp-torro-forms-date-select' ),
			'month' => __( 'Month', 'kraftner-wp-torro-forms-date-select' ),
			'day'   => __( 'Day', 'kraftner-wp-torro-forms-date-select' ),
		);
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $settings['required'] ) && ( empty( $year ) || empty( $month ) || empty( $day ) ) ) {
			return $this->create_error( 'value_required', __( 'You must enter something here.', 'kraftner-wp-torro-forms-date-select' ), $parsed_value );
		}

		if ( $month < 1 || $month > 12 ) {
			return $this->create_error( 'month_out_of_boundaries', __( 'The value for the month must be between 1 and 12.', 'kraftner-wp-torro-forms-date-select' ), $parsed_value );
		}

		if ( $day < 1 || $month > 31 ) {
			return $this->create_error( 'day_out_of_boundaries', __( 'The value for the day must be between 1 and 31.', 'kraftner-wp-torro-forms-date-select' ), $parsed_value );
		}
	...
	protected function bootstrap() {
		...
		$this->settings_fields['min_year'] = array(
			'section'       => 'settings',
			'type'          => 'number',
			'label'         => __( 'Minimum year', 'kraftner-wp-torro-forms-date-select' ),
		...
		$this->settings_fields['max_year'] = array(
			'section'       => 'settings',
			'type'          => 'number',
			'label'         => __( 'Maximum year', 'kraftner-wp-torro-forms-date-select' ),
			'input_classes' => array( 'small-text' ),
			'default'       => current_time( 'Y' ),
			'min'           => 1,
			'step'          => 1,
